 ...
 * 
 * There are two implementations for the image downloader challenge. The
 * sequential implementation which does one image at a time, and the parallel
 * implementation which uses futures to speed up the problem.
 * 
 * This challenge is an I/O Bound challenge. It's fun to open the task manager
 * and compare the WiFi traffic between the two implementations.
 */
#include <cstring>
...
size_t parallel_image_downloader(int num_images) {
    size_t total_bytes = 0;
    // The download image helper is a method that we can call async for each
    // image since they're separate independent tasks. We'll use futures for this.
